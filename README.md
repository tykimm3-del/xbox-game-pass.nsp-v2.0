...
- **NRO:** copy `green-nx.nro` to `sdmc:/switch/xbox/green-nx.nro` and launch it through the Homebrew Menu in title mode. Applet mode does not provide enough memory for hardware video decoding.
- **NSP:** build the final installable package with the `Build installable Xbox NSP` workflow and the encrypted GitHub Actions Secret instructions in `README_NSP_KO.md`, then use the package only on a CFW system you own and manage.
- Sign in once with the device code shown on screen (`microsoft.com/link`). Tokens and the game list are cached in `sdmc:/switch/xbox/`.
